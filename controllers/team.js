 ...
 * POST /team
 */
exports.teamPost = function(req, res) {
...
exports.getTeamMembers = function(req, res) {
  ...
  Team.findOne({ _id: teamId }).populate("members").exec(function(err, team) {
    if (!team.isTeamMember(req.user._id)) {
      return res
        .status(403)
        .send({ msg: "You have to be an team member to perform this action." });
    }
 ...
 */
exports.getTeamPendingMembers = function(req, res) {
  var teamId = req.params.id;

  if (!req.user.isAdmin) {
    return res
      .status(403)
      .send({ msg: "You have to be the team admin to perform this action." });
  }
    ...
    .exec(function(err, team) {
      if (!team.isTeamMember(req.user._id)) {
        return res
          .status(403)
          .send({
            msg: "You have to be an team member to perform this action."
          });
      }
 ...
 */
exports.patchTeamPendingMembers = function(req, res) {
  var teamId = req.params.id;
  ...
  Team.findOne({ _id: teamId }).exec(function(err, team) {
    ...
    if (action === "accept") {
      Team.findOneAndUpdate(
        teamId,
        { $pull: { pending_members: userId }, $addToSet: { members: userId } },
        { safe: true, new: true },
        function(err, team) {
          if (err) {
            return res.status(400).send({ error: err });
      ...
      User.findByIdAndRemove({ _id: userId }, function(err, user) {
        ...
        if (action === "reject") {
          ...
          Team.findOneAndUpdate(
            ...
            {
              $pull: { pending_members: userId },
              $addToSet: { blocked_users: user.email }
            },
            { safe: true, new: true },
            function(err, team) {
              if (err) {
